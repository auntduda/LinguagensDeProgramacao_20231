...
aprovadosOrdemDeMedia ((name, n1, n2):pls) = sortBy (\(_, y1) (_,y2) -> if y1>=y2 then LT else GT) (filter (\(_, y) -> y >= 5.0) ((name, nf) : aprovadosOrdemDeMedia pls))
                                            where nf = (n1+n2)/2
...
-- perguntar pro Luiz como isso aqui funciona
